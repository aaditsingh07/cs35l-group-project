...
export default function AdminGroupsPage() {
  ...
  async function fetchAll() {
    ...
    try {
      ...
      if (!uRes.ok || !gRes.ok || !tRes.ok) { setError("Failed to load data."); return; }
  ...
  async function createGroup(e) {
    ...
    if (res.ok) { setNewGroupName(""); setNewGroupDesc(""); fetchAll(); }
  ...
  async function handleAutoAssign() {
    ...
    else { setAutoMsg(`Done — created ${data.groupsCreated} groups for ${data.usersAssigned} users.`); fetchAll(); }
  }

  const allVisibleSelected = users.length > 0 && users.every((u) => selectedIds.has(u.id));

  const TABS = ["users", "groups", "tasks"];
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: "2rem", fontFamily: "sans-serif" }}>
      ...
      <p>Manage users, groups, and tasks across the platform.</p>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
        {TABS.map((t) => (
          ...
          >
            {t === "users" ? "👥 Users" : t === "groups" ? "🛡️ Groups" : "⚡ Tasks"}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <table border="1" cellPadding="8" style={{ borderCollapse: "collapse", width: "100%" }}>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  <td><input type="checkbox" checked={selectedIds.has(u.id)} onChange={() => toggleSelected(u.id)} /></td>
                  <td>{u.id}</td>
                  <td>{u.name}</td>
                  <td>{u.email}</td>
        ...
        <>
          <form
            ...
            style={{
              ...
              background: "#f7f7f7", border: "1px solid #ccc", borderRadius: "8px",
            }}
          >
            <strong>New group:</strong>
            <input type="text" placeholder="Name (required)" value={newGroupName}
              onChange={(e) => setNewGroupName(e.target.value)} style={{ padding: "0.4rem" }} required />
            <input type="text" placeholder="Description (optional)" value={newGroupDesc}
              onChange={(e) => setNewGroupDesc(e.target.value)} style={{ padding: "0.4rem", flex: 1 }} />
          ...
          <div
            style={{
              ...
              background: "#f0f7ff", border: "1px solid #99c", borderRadius: "8px",
            }}
          >
            <strong>Auto-assign unassigned users:</strong>
            <label>
              Group size:
              <input type="number" min="2" max="10" value={autoSize}
          ...
          <table border="1" cellPadding="8" style={{ borderCollapse: "collapse", width: "100%" }}>
            <thead>
              <tr><th>ID</th><th>Name</th><th>Description</th><th>Members</th><th>Created</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {groups.map((g) => (
                <tr key={g.id}>
                  <td>{g.id}</td><td>{g.name}</td><td>{g.description ?? "—"}</td>
                  <td>{g.member_count}</td><td>{g.created_at}</td>
      ...
      {tab === "tasks" && (
        tasks.length === 0 ? <p>No tasks found.</p> : (
          <table border="1" cellPadding="8" style={{ borderCollapse: "collapse", width: "100%" }}>
            ...
            <tbody>
              {tasks.map((t, i) => (
                <tr key={i}>{Object.values(t).map((v, j) => <td key={j}>{String(v ?? "—")}</td>)}</tr>
              ))}
            </tbody>
          </table>
        )
